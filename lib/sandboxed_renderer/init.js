...
const {
  preloadSrc, preloadError, modules, process: processProps
} = electron.ipcRenderer.sendSync('ELECTRON_BROWSER_SANDBOX_LOAD')

function enableModule (name, enable) {
  if (enable) {
    let descriptor = Object.getOwnPropertyDescriptor(electron, name)
    descriptor.configurable = false
    Object.defineProperty(electron, name, descriptor)
  } else {
    delete electron[name]
  }
}

for (const [name, enable] of Object.entries(modules)) {
  enableModule(name, enable)
}
